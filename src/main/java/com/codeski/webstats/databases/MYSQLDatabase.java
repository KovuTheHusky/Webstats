...
public class MYSQLDatabase extends Database {
	private final String[] tables = {
			...
			"CREATE TABLE IF NOT EXISTS ws_fishes (fish_casts INTEGER, fish_catches INTEGER, fish_player BIGINT PRIMARY KEY)",
	...
	public void playerJoined(Player player) {
		int count = this.update("UPDATE ws_players SET player_joined = player_joined + 1 WHERE player_name = '" + player.getName() + "'");
		if (count < 1)
			this.update("INSERT INTO ws_players (player_joined, player_name, player_uuid) VALUES (1, '" + player.getName() + "', '" + player.getUniqueId() + "')");
	}

	@Override
	public void playerQuit(Player player) {
		this.update("UPDATE ws_players SET player_quit = player_quit + 1 WHERE player_name = '" + player.getName() + "'");
	}
}

...
$res = $mysqli->query("SELECT damage_time FROM ws_damages ORDER BY damage_time ASC LIMIT 1");
...
$res = $mysqli->query("SELECT damage_time FROM ws_damages ORDER BY damage_time DESC LIMIT 1");
...
if ($by == 'type')
	$donut = $mysqli->query("SELECT type_name, ROUND(SUM(damage_amount)) FROM ws_damages JOIN ws_damage_types ON type_id = damage_type WHERE damage_time >= '{$dt[$time]['start']->format('Y-m-d H:i:s')}' GROUP BY damage_type ORDER BY SUM(damage_amount) DESC")->fetch_all();
else if ($by == 'player')
	$donut = $mysqli->query("SELECT player_name, ROUND(SUM(damage_amount)) FROM ws_damages JOIN ws_players ON player_id = damage_damager_player WHERE damage_time >= '{$dt[$time]['start']->format('Y-m-d H:i:s')}' GROUP BY player_name ORDER BY SUM(damage_amount) DESC")->fetch_all();
...
if ($by == 'type')
	$res = $mysqli->query("SELECT type_name AS 'event', ROUND(SUM(damage_amount)) AS 'count', damage_time AS 'time' FROM ws_damages JOIN ws_damage_types ON damage_type = type_id WHERE damage_time > '{$dt[$time]['start']->format('Y-m-d H:i:s')}' GROUP BY type_name, {$dt[$time]['interval']}(damage_time) ORDER BY damage_time ASC");
else if ($by == 'player')
$res = $mysqli->query("SELECT player_name AS 'event', ROUND(SUM(damage_amount)) AS 'count', damage_time AS 'time' FROM ws_damages JOIN ws_players ON player_id = damage_damager_player WHERE damage_time > '{$dt[$time]['start']->format('Y-m-d H:i:s')}' GROUP BY player_name, {$dt[$time]['interval']}(damage_time) ORDER BY damage_time ASC");
if ($res->num_rows < 1)
	$line = array();
...
<?php require_once $_SERVER['DOCUMENT_ROOT'] . '/assets/includes/navigation.php'; ?>
  <nav class="small">
    <ul>
      ...
      <li<?php if ($time == 'day') echo ' class="selected"'; ?>><?php if ($valid['day']) { ?><a href="/damages/day/<?php echo $by; ?>">Day</a><?php } else { ?><span style="text-decoration: line-through;">Day</span><?php } ?></li>
      <li<?php if ($time == 'week') echo ' class="selected"'; ?>><?php if ($valid['week']) { ?><a href="/damages/week/<?php echo $by; ?>">Week</a><?php } else { ?><span style="text-decoration: line-through;">Week</span><?php } ?></li>
      <li<?php if ($time == 'month') echo ' class="selected"'; ?>><?php if ($valid['month']) { ?><a href="/damages/month/<?php echo $by; ?>">Month</a><?php } else { ?><span style="text-decoration: line-through;">Month</span><?php } ?></li>
      <li<?php if ($time == 'year') echo ' class="selected"'; ?>><?php if ($valid['year']) { ?><a href="/damages/year/<?php echo $by; ?>">Year</a><?php } else { ?><span style="text-decoration: line-through;">Year</span><?php } ?></li>
      <li<?php if ($time == 'all') echo ' class="selected"'; ?>><a href="/damages/all/<?php echo $by; ?>">All</a></li>
    </ul>
    <ul>
      <li>By:</li>
      <li><span style="text-decoration: line-through;">Event</span></li>
      <li<?php if ($by == 'player') echo ' class="selected"'; ?>><a href="/damages/<?php echo $time; ?>/player">Player</a></li>
      <li<?php if ($by == 'type') echo ' class="selected"'; ?>><a href="/damages/<?php echo $time; ?>/type">Type</a></li>
    </ul>

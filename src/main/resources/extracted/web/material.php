...
if (!empty($event_raw)) {
	for ($i = 0; $i < 24; ++$i)
		foreach ($event_raw as $e)
			$over_time[$i][$e] = 0;
	foreach ($event_raw as $e) {
		$res = $mysqli->query("SELECT COUNT(0) as c, material_time as t FROM ws_materials JOIN ws_material_events ON material_event = event_id JOIN ws_material_types ON material_type = type_id WHERE type_name = '{$material}' AND event_name = '{$e}' GROUP BY HOUR(t)");
		while ($row = $res->fetch_assoc()) {
